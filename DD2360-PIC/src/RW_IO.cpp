...
void readInputFile(struct parameters *param, int argc, char **argv) {
  ...
  /** Smoothing value*/
  param->SmoothValue =
      config.read<double>("SmoothValue", 0.5); // between 0 and 1, typically 0.5
...
void saveParameters(struct parameters *param) {
  ...
  buffer << "-----------------------------" << std::endl;
  buffer << "- sputniPIC Sim. Parameters -" << std::endl;
  buffer << "-----------------------------" << std::endl;

  buffer << std::endl;

  buffer << "Number of species    = " << param->ns << std::endl;
  for (int i = 0; i < param->ns; i++)
    buffer << "Number of particles of species " << i << " = " << param->np[i]
           << "\t (MAX = " << param->npMax[i] << ")"
           << "  QOM = " << param->qom[i] << std::endl;
  buffer << "---------------------------" << std::endl;
  buffer << "x-Length                 = " << param->Lx << std::endl;
  buffer << "y-Length                 = " << param->Ly << std::endl;
  buffer << "z-Length                 = " << param->Lz << std::endl;
  buffer << "Number of cells (x)      = " << param->nxc << std::endl;
  buffer << "Number of cells (y)      = " << param->nyc << std::endl;
  buffer << "Number of cells (z)      = " << param->nzc << std::endl;
  buffer << "---------------------------" << std::endl;
  buffer << "Time step                = " << param->dt << std::endl;
  buffer << "Number of cycles         = " << param->ncycles << std::endl;
  buffer << "---------------------------" << std::endl;
  for (int is = 0; is < param->ns; is++)
    buffer << "rho init species " << is << " = " << param->rhoINIT[is]
           << std::endl;
  buffer << "current sheet thickness  = " << param->delta << std::endl;
  buffer << "B0x                      = " << param->B0x << std::endl;
  buffer << "BOy                      = " << param->B0y << std::endl;
  buffer << "B0z                      = " << param->B0z << std::endl;
  buffer << "---------------------------" << std::endl;
  buffer << "Smooth ON                = " << param->SmoothON << std::endl;
  buffer << "GMRES error tolerance    = " << param->GMREStol << std::endl;
  buffer << "CG error tolerance       = " << param->CGtol << std::endl;
  buffer << "Mover error tolerance    = " << param->NiterMover << std::endl;
  buffer << "---------------------------" << std::endl;
  buffer << "Results saved in: " << param->SaveDirName << std::endl;
  buffer << "Restart saved in: " << param->RestartDirName << std::endl;
  buffer << "---------------------" << std::endl;
...
void VTK_Write_Vectors(int cycle, struct grid *grd, struct EMfield *field) {
  ...
  buffer << "# vtk DataFile Version 1.0" << std::endl;
  buffer << "E field" << std::endl;
  buffer << "ASCII" << std::endl;
  buffer << "DATASET STRUCTURED_POINTS" << std::endl;
  buffer << "DIMENSIONS " << (nxn - 3) << " " << (nyn - 3) << " " << (nzn - 3)
         << std::endl;
  buffer << "ORIGIN " << 0.0 << " " << 0.0 << " " << 0.0 << std::endl;
  buffer << "SPACING " << dx << " " << dy << " " << dz << std::endl;
  buffer << "POINT_DATA " << (nxn - 3) * (nyn - 3) * (nzn - 3) << std::endl;
  buffer << "VECTORS E float" << std::endl;
  ...
  buffer << "SPACING " << dx << " " << dy << " " << dz << std::endl;
  buffer << "POINT_DATA " << (nxn - 3) * (nyn - 3) * (nzn - 3) << std::endl;
  buffer << "VECTORS B float" << std::endl;
...
void VTK_Write_Scalars(int cycle, struct grid *grd,
  ...
  std::ofstream my_file(temp.c_str());
  buffer << "# vtk DataFile Version 1.0" << std::endl;
  buffer << "Electron Density - Current Sheet " << std::endl;
  buffer << "ASCII" << std::endl;
  buffer << "DATASET STRUCTURED_POINTS" << std::endl;
  buffer << "DIMENSIONS " << (nxn - 3) << " " << (nyn - 3) << " " << (nzn - 3)
         << std::endl;
  ...
  buffer << "SPACING " << dx << " " << dy << " " << dz << std::endl;
  buffer << "POINT_DATA " << (nxn - 3) * (nyn - 3) * (nzn - 3) << std::endl;
  buffer << "SCALARS rhoe float" << std::endl;
  buffer << "LOOKUP_TABLE default" << std::endl;

  for (int k = 1; k < nzn - 2; k++)
    for (int j = 1; j < nyn - 2; j++)
      for (int i = 1; i < nxn - 2; i++)
        buffer << ids[0].rhon[i][j][k] << std::endl;
  ...
  std::ofstream my_file2(temp.c_str());
  buffer << "# vtk DataFile Version 1.0" << std::endl;
  buffer << "Ion Density - Current Sheet" << std::endl;
  buffer << "ASCII" << std::endl;
  buffer << "DATASET STRUCTURED_POINTS" << std::endl;
  buffer << "DIMENSIONS " << (nxn - 3) << " " << (nyn - 3) << " " << (nzn - 3)
         << std::endl;
  ...
  buffer << "SPACING " << dx << " " << dy << " " << dz << std::endl;
  buffer << "POINT_DATA " << (nxn - 3) * (nyn - 3) * (nzn - 3) << std::endl;
  buffer << "SCALARS rhoi float" << std::endl;
  buffer << "LOOKUP_TABLE default" << std::endl;

  for (int k = 1; k < nzn - 2; k++)
    for (int j = 1; j < nyn - 2; j++)
      for (int i = 1; i < nxn - 2; i++)
        buffer << ids[1].rhon[i][j][k] << std::endl;
  ...
  buffer << "# vtk DataFile Version 1.0" << std::endl;
  buffer << "Net Charge Density" << std::endl;
  buffer << "ASCII" << std::endl;
  buffer << "DATASET STRUCTURED_POINTS" << std::endl;
  buffer << "DIMENSIONS " << (nxn - 3) << " " << (nyn - 3) << " " << (nzn - 3)
         << std::endl;
  ...
  buffer << "SPACING " << dx << " " << dy << " " << dz << std::endl;
  buffer << "POINT_DATA " << (nxn - 3) * (nyn - 3) * (nzn - 3) << std::endl;
  buffer << "SCALARS rhonet float" << std::endl;
  buffer << "LOOKUP_TABLE default" << std::endl;

  for (int k = 1; k < nzn - 2; k++)
    for (int j = 1; j < nyn - 2; j++)
      for (int i = 1; i < nxn - 2; i++)
        buffer << idn->rhon[i][j][k] << std::endl;
