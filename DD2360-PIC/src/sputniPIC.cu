...
int main(int argc, char **argv) {
  ...
  for (int is = 0; is < param.ns; is++) {
    // Allocate fields for the current species
    FPinterp *d_rhon_flat;

    int interp_size = grd.nxn * grd.nyn * grd.nzn * sizeof(FPinterp);

    gpuErrchk(cudaMalloc(&d_rhon_flat, interp_size));

    gpuErrchk(cudaMemcpy(&(d_ids[is].rhon_flat), &d_rhon_flat,
  ...
  gpuErrchk(
      cudaMemcpy(d_idn, &idn, sizeof(interpDensNet), cudaMemcpyHostToDevice));
  ...
  for (int is = 0; is < param.ns; is++) {
    ...
    gpuErrchk(cudaMemcpy(&d_rhon_flat, &(d_ids[is].rhon_flat),
                         sizeof(FPinterp *), cudaMemcpyDeviceToHost));

    // Copy host data to device fields
    gpuErrchk(cudaMemcpy(d_rhon_flat, ids[is].rhon_flat, interp_size,
                         cudaMemcpyHostToDevice));
  }
  ...
  for (int is = 0; is < param.ns; is++) {
    ...
    // Retrieve device pointers for fields from the device structure
    FPpart *d_x, *d_y, *d_z, *d_u, *d_v, *d_w;
    FPinterp *d_q;

    gpuErrchk(cudaMemcpy(&d_x, &(d_part[is].x), sizeof(FPpart *),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(&d_y, &(d_part[is].y), sizeof(FPpart *),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(&d_z, &(d_part[is].z), sizeof(FPpart *),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(&d_u, &(d_part[is].u), sizeof(FPpart *),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(&d_v, &(d_part[is].v), sizeof(FPpart *),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(&d_w, &(d_part[is].w), sizeof(FPpart *),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(&d_q, &(d_part[is].q), sizeof(FPinterp *),
                         cudaMemcpyDeviceToHost));
  ...
  for (int cycle = param.first_cycle_n;
    ...
    for (int is = 0; is < param.ns; is++) {
      ...
      gpuErrchk(cudaMemcpy(&d_rhon_flat, &(d_ids[is].rhon_flat),
                           sizeof(FPinterp *), cudaMemcpyDeviceToHost));
      gpuErrchk(cudaMemset(d_rhon_flat, 0, interp_size));
    }
    ...
    // Do not understand why this needs to be done here
    gpuErrchk(cudaMemcpy(&d_rhon_flat_n, &d_idn->rhon_flat, sizeof(FPinterp *),
                         cudaMemcpyDeviceToHost));
    ...
    if (cycle % param.FieldOutputCycle == 0) {
      ...
      // Copy EMfield to host
      gpuErrchk(
          cudaMemcpy(&field, d_field, sizeof(EMfield), cudaMemcpyDeviceToHost));
      ...
      for (int is = 0; is < 2; is++) {
        gpuErrchk(cudaMemcpy(&ids[is], &d_ids[is], sizeof(interpDensSpecies),
                             cudaMemcpyDeviceToHost));
        ...
        gpuErrchk(cudaMemcpy(&d_rhon_flat, &(d_ids[is].rhon_flat),
                             sizeof(FPinterp *), cudaMemcpyDeviceToHost));
        // Copy field data back to host arrays
        int interp_size = grd.nxn * grd.nyn * grd.nzn * sizeof(FPinterp);

        gpuErrchk(cudaMemcpy(ids[is].rhon_flat, d_rhon_flat, interp_size,
                             cudaMemcpyDeviceToHost));
      }

      // Copy interpDensNet to host
      gpuErrchk(cudaMemcpy(&idn, d_idn, sizeof(interpDensNet),
                           cudaMemcpyDeviceToHost));
  ...
  std::cout << "   Dens Time / Cycle    (s) = " << eDens / param.ncycles
            << std::endl;
  std::cout << "   IO Time / Cycle    (s) = " << eWrite / param.ncycles
            << std::endl;
  std::cout << "**************************************" << std::endl;
